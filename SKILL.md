...
> 让新会话 AI 第一秒接续项目心智、不重踩坑。**只做读代码做不到的事 —— 采集注入不可推导知识**;模块图/位置/调用关系都可推导 → 读代码/codegraph,本 skill 不维护任何骨架文档(手写只会腐化)。
...
**先路由再写(按耐久度,auto-memory 垫底)**:出错点代码注释/类型 → 写代码 · LLM 行为纠正 → 全局 `~/.claude/rules/` · 个人偏好 → `~/.claude/local/CLAUDE.md` · 项目约定/决策 → 项目 CLAUDE.md · UI/平台 idiom → dev-cases · **连代码+注释都推不出的硬件/SDK/协议不变量 → auto-memory**(一事一条+Why,坑用 symptom→cause→fix;写文件必加 MEMORY.md 索引行,`/pmm check --fix` 兜底;永不自动 commit) · 易失 → current-state · 文件改动 → 不记。
...
3. **不维护骨架/流程/位置文档** —— 结构交给读代码/grep(大库才用 codegraph:skill 内 vendored 零安装 `python3 cli.py …`,外部 `pipx install "git+https://github.com/PsChina/project-mental-model.git#subdirectory=tools/codegraph"`)。
