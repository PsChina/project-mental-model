...
description: 为项目维护"开局即接续"的心智:① CLAUDE.md 宪法 ② 不可推导认知进 auto-memory ③ current-state 易失态;结构交给读代码/codegraph、不手写。新会话接续陌生/久未碰的项目、里程碑收尾刷新心智、或显式 `/pmm`(建)·`log`(沉淀)·`check`(体检)时用;单点问答 / 写功能等能从代码推出来的不用。首次 `bootstrap-verify.sh --install`。
---
...
- **B 沉淀(自主 · 持续)**(触发:**每条含决策 / 纠正 / 新约束的用户消息——发消息即轻量判断,不等里程碑、不等对话结束** —— 全局 `UserPromptSubmit` hook `pmm-capture-detect.js` 检出信号即喂入静默指令;也含 `log` 显式调用)
  **为什么钉在「用户发消息」这个点**:幸存者偏差 —— AI 做得好用户直接走人、根本不会有下一条消息(像商品好评);用户发消息(尤其纠正,即「差评」侧)才是最高价值信号。所以不等里程碑、不等人调用,在消息提交时就自主判断,记录与更新都在这完成。
  动作:**主 agent 不得自己把静默候选直接写进 store**。回顾上下文做廉价初筛(默认不写):明显可推导 / 一次性 / 刚修完的 bug → 直接丢弃,不送审。剩下的疑似候选 **必须派独立审核员** `Task(subagent_type="memory-gatekeeper")` 过审 —— 把候选 + 上下文(用户原话或你的发现、相关 file:line、是否已写过代码注释)+ 本项目 MEMORY.md / current-state 路径 传给它;它按双闸独立判 REJECT/UPDATE/ADD、默认拒、过审才由它落盘并同步索引。你只把它的回执当结果,全程静默、不向用户提问。
  **为什么必须经独立审核员**:干活的主 agent 有偏见(刚发现觉得很香 + 幸存者偏差),自己拍板写 = 噪音入库的根源。审核员没参与本次工作、只认 bar、默认拒,把"写不写"从有偏见的人手里拿走。
  **你回合内读代码 / 调试发现的洞见也走同一道闸**:别"当场就记" —— 优先把 why 写进**出错点代码注释**(bar 路由铁律:能进注释就别进 memory);确认写不进注释、又是会复发的不变量,才作为候选送审核员。刚修完的 bug 的根因几乎都属于"进注释、不进 memory"。
...
3. **增量优先于全量**,新覆盖旧(recency-wins);**永不自动 commit / push**;不记文件清单。**严禁流水账** —— current-state 答"现在什么成立",不答"改了什么 / 何时改":
   - 顶部**只一行日期戳**(`Last verified: YYYY-MM-DD ｜ 验证: grep <符号>`):不加 run 计数(#6/#5)、不嵌"本轮做了 X"叙述、不另开 `Last pruned` / `Last session update` 第二三行;
   - **不记**行号漂移校正(A→B)、commit hash、"其余 commits 均 cleanup"、"无新增 memory(git 可推导)" —— 这些过程 / 决策叙述全是 git 的职责;
   - 删除即**静默删行**:消除一条 = 该行从此消失,不留删除线 graveyard / "✅已消除(日期)" / "本轮已复核消除"注脚。
