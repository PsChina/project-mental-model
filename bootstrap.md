...
> **一条命令**:`bash templates/bootstrap-verify.sh --install`(幂等装齐核心链、只补缺不覆盖,装完自动复核;日常体检跑无参版,`--fix` 补孤儿索引)→ 然后**重开一次会话**(命令别名才加载)。**均不自动 commit。** `--install` 已解"要 `/pmm` 才能建 `/pmm`"死循环;纯 Windows 无 bash → 按下面手动核。
